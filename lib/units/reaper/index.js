        ...
        return Promise.promisify(cursor.toArray, cursor)()
          .then(function(list) {
            ...
            list.forEach(function(device) {
              log.info("load initial device: " + device.serial + ", " + device.present + ", " + device.ready + ", " + device.status)
  ...
  function listenToChanges() {
    sub.on('message', wirerouter()
      .on(wire.DeviceIntroductionMessage, function(channel, message) {
        log.info(message.serial + 'DeviceIntroductionMessage')
      ...
      .on(wire.DeviceHeartbeatMessage, function(channel, message) {
        log.info(message.serial + 'DeviceHeartbeatMessage')
        ttlset.bump(message.serial, Date.now())
      })
      .on(wire.DeviceAbsentMessage, function(channel, message) {
        log.info(message.serial + 'DeviceAbsentMessage')
        ttlset.drop(message.serial, TtlSet.SILENT)
      })
      .handler())
  }
