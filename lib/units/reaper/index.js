  ...
  function listenToChanges() {
    sub.on('message', wirerouter()
      ...
      .on(wire.DeviceHeartbeatMessage, function(channel, message) {
        //在收到心跳时上报设备的STF状态
        var url = 'http://booster.58.com/manage/updateStfStatus?serial=' + message.serial
        request(url, function (error, response, body) {
          if (!error && response.statusCode == 200) {
            log.info('report device "%s" status: %s', message.serial, JSON.stringify(body))
          } else {
            log.fatal('report device "%s" status: %s', message.serial, error)
          }
        })
        ttlset.bump(message.serial, Date.now())
      })
